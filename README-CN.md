...
用户自然语言描述
  ...
  dedup_papers              跨搜索源去重
        │
        ▼
  filter_papers    使用 GLM-4.7-FlashX 基于原始问题描述和摘要过滤低相关论文
        │
        ▼
...
GLM_API_KEY=...         # 智谱 GLM — 用于检索后的相关性过滤
...
`DEEPSEEK_API_KEY` 在使用 `--describe` 模式时必填。
`GLM_API_KEY` 在检索后的相关性过滤步骤中必填。
